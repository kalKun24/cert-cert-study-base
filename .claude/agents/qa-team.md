...
1. **検証対象を特定する**（優先順）:
   ...
   2. 指定が無ければ現ブランチの差分: `git diff main...HEAD`（develop 起点のブランチなら `develop...HEAD`）と `git log main..HEAD --oneline`（同）で範囲を把握する
...
**フォールバック**: API Tester が「実行不可」だった場合（Docker 停止中など）も残りのレポートで Phase 3 へ進む。ただし API 変更を含む場合、判定は自動的に NEEDS WORK になる（Reality Checker の判定基準に含まれている）。なお `Bash(curl:*)`（または localhost 限定の許可）が permissions に無い環境では、API Tester の curl が許可プロンプトで停止しうる。

**片付け**: API Tester が「自分が起動した（down 未実施）」と報告した場合、Reality Checker の完了後にあなたが `make down` を実行する（Reality Checker の抜き取り検証で環境を使えるよう、Phase 3 より前に down しない）。
