...
- [ ] 問題一覧ページ: 問題のタイトル・タグ・作成日を一覧表示し、タグフィルタ・キーワード検索ができる
- [ ] 問題詳細ページ: Markdownをレンダリングして表示する（body / answer / explanation / memo）
- [ ] 問題作成ページ: Markdownエディタで入力し、タグを複数選択して保存できる
- [ ] 問題編集ページ: 既存データを読み込んでMarkdownエディタで編集・保存できる
- [ ] 問題削除: 確認ダイアログ付きで削除できる
- [ ] 未認証ユーザーはログイン画面にリダイレクトされる
- [ ] ESLint / Prettier がエラーなく通る

---

## サブチケット（コミット単位）

- [ ] `chore(frontend): APIクライアント（fetch wrapper）とルーティング基盤を実装`
- [ ] `feat(frontend): 問題一覧ページを実装（タグフィルタ・キーワード検索含む）`
- [ ] `feat(frontend): 問題詳細ページ（Markdownレンダリング）を実装`
- [ ] `feat(frontend): 問題作成・編集ページ（Markdownエディタ + タグ選択）を実装`
- [ ] `feat(frontend): 問題削除機能（確認ダイアログ）を実装`
