...
- [ ] `chore(backend): Goモジュールの初期化とディレクトリ構成を作成`
- [ ] `chore(frontend): React + TypeScriptプロジェクトを初期化`
- [ ] `chore(infra): docker-compose.ymlとDockerfileを作成`
- [ ] `chore(make): Makefileにローカル開発コマンドを定義`
- [ ] `docs(api): openapi.yamlのスケルトンを作成`
- [ ] `chore(ci): GitHub ActionsのCI/CDワークフローを追加`
