...
| PR番号 | #66 |
| PRリンク | https://github.com/kalKun24/cert-cert-study-base/pull/66 |
...
- [x] `main` へのマージ後、CI（既存）→ CD（新規）の順でパイプラインが実行される
- [x] CD ジョブがサービスアカウントキーなし（Workload Identity Federation）で GCP 認証できる
- [x] バックエンド・フロントエンド両方が自動デプロイされる
- [x] デプロイ完了後に Cloud Run のサービス URL がジョブのサマリーに出力される
- [x] シークレット（JWT_SECRET 等）はリポジトリの Secrets に平文で保存しない
