...
- [x] Enter キーで自然に改行でき、2回 Enter で段落区切りとなる Markdown の改行ルールが直感的に操作できること
- [x] 見出し（#〜###）・太字・斜体・コードブロック・リストのシンタックスハイライトが有効なこと
- [x] ツールバーに太字・斜体・リンク・テーブル・コードブロック・チェックリストの挿入ボタンがあること
- [x] 編集エリアとプレビューを左右に分割表示できること（スプリットビュー）
- [x] プレビューは既存の `react-markdown` + `rehype-sanitize` を使い、XSS 対策が維持されること
- [x] Teal テーマとデザイン的に整合するスタイルであること（CSS カスタムプロパティ使用）
- [x] `MarkdownEditor.tsx` として共通コンポーネントに切り出し、`QuestionCreatePage` / `QuestionEditPage` / `NoteCreatePage` / `NoteEditPage` の 4 ページすべてで利用されること
- [x] モバイル表示でもエディタとプレビューが（タブ切り替えなどで）参照できること
- [x] 既存の `@uiw/react-md-editor` への依存が削除されること（package.json から除去）
- [x] ESLint / Prettier のエラーがないこと
