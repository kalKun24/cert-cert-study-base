...
- [ ] GCP 本番環境で非管理者ユーザーがチーム一覧を取得できる
- [ ] GCP 本番環境で非管理者ユーザーが問題の閲覧・作成・編集ができる

---

## サブチケット（コミット単位）

- [ ] `fix(firestore): コレクショングループ・複合インデックスを定義する firestore.indexes.json を追加`
- [ ] `fix(cd): CD ワークフローに Firestore インデックス自動デプロイステップを追加`
