...
| PR番号 | #15 |
| PRリンク | https://github.com/kalKun24/cert-cert-study-base/pull/15 |
...
- [x] admin のみ `/admin/users` にアクセスできる（他ロールはアクセス時に 403 / リダイレクト）
- [x] ユーザー一覧が表示され、有効 / 停止中の状態が視覚的に区別できる
- [x] ユーザー作成フォームから新規ユーザーを作成できる
- [x] ユーザー情報（display_name / email / role）を編集できる
- [x] 有効化・停止の切り替えができる（確認ダイアログあり）
- [x] ユーザー削除ができる（確認ダイアログあり）

---

## サブチケット（コミット単位）

- [x] `feat(frontend): ユーザー一覧画面を実装`
- [x] `feat(frontend): ユーザー作成・編集フォームを実装`
- [x] `feat(frontend): ユーザー有効化・停止・削除操作を実装`
- [x] `feat(frontend): admin ルートガード（非 admin のアクセス制御）を実装`
