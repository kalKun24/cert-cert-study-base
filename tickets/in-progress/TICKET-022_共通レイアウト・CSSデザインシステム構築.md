...
- [ ] `frontend/src/styles/global.css` が作成され、`main.tsx` で import されている
- [ ] CSS カスタムプロパティ（デザイントークン）が `:root` に定義されている
...
- [ ] グローバルリセットスタイル（`box-sizing: border-box`・body のデフォルトスタイル）が適用されている
- [ ] アプリレイアウト（`.app-layout` / `.app-body` / `.main-content`）がデスクトップ・タブレット・モバイルで正しく表示される
  - デスクトップ: NavBar(sticky, 56px) + Sidebar(240px) + Main の3ペイン
  - モバイル: サイドバー非表示、NavBar にハンバーガーボタンを追加してサイドバーをオーバーレイ表示
- [ ] NavBar はブランド名・ユーザー情報（表示名・ロールバッジ）・プロフィール/ログアウトボタンのみに整理される（ナビリンクはサイドバーに集約）
- [ ] サイドバーに `/teams` リンクが追加され、ナビゲーションリンクが一元管理されている（ホーム・問題一覧・タグ管理・チーム一覧・ユーザー管理）
- [ ] サイドバーのリンクに `NavLink` の `active` クラスが機能しアクティブ状態が視覚的に区別できる
- [ ] `role-badge` に `data-role` 属性を付与し、admin は赤系・その他は青系でスタイルが異なる
- [ ] モバイル時に NavBar のハンバーガーボタン（☰）をタップするとサイドバーがオーバーレイ表示され、外側タップ・ESC キーで閉じる
- [ ] WCAG AA 準拠: ボタン色(#f9bc60)と背景(#004643)のコントラスト比 4.5:1 以上
- [ ] `@media (prefers-reduced-motion: reduce)` によるモーション抑制が適用されている
- [ ] `.sr-only` クラスが定義されている
...
- [ ] `feat(styles): CSSデザイントークンとグローバルスタイルを追加`
- [ ] `feat(layout): アプリレイアウト・NavBar・サイドバーのスタイルを追加`
- [ ] `refactor(layout): NavBarのナビリンクを削除しサイドバーに集約、/teams リンクを追加`
- [ ] `feat(layout): モバイル用ハンバーガーメニューとサイドバーオーバーレイを実装`
- [ ] `feat(styles): ボタン・フォーム・アラート・カードのグローバルスタイルを追加`
- [ ] `feat(styles): バッジ・ステータス・ページタイトルのスタイルを追加`
