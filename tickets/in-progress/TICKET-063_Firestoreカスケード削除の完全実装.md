...
- [ ] `team_repository.go` の Delete() が questions/{id}/comments、notes/{id}/comments、tags/ を再帰的に削除する
- [ ] `question_repository.go` の Delete() が questions/{id}/comments サブコレクションを削除する
- [ ] `note_repository.go` の Delete() が notes/{id}/comments サブコレクションを削除する
- [ ] `tag_repository.go` の Delete() の使用中チェックがノートへの参照も確認する（noteRepo.FindByTagID 相当）
- [ ] 削除後に孤立ドキュメントが残らないことをユニットテスト（Firestore エミュレータ or モック）で確認する

---

## サブチケット（コミット単位）

- [ ] `fix(firestore): question / note の Delete() にコメントサブコレクション削除を追加`
- [ ] `fix(firestore): team の Delete() にカスケード削除（questions/notes/tags とその入れ子）を追加`
- [ ] `fix(firestore): tag の Delete() 使用中チェックにノート参照確認を追加`
- [ ] `test(firestore): カスケード削除の孤立ドキュメント残存チェックをテストに追加`
