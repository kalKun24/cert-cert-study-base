...
[![CD](https://github.com/kalKun24/cert-study-base/actions/workflows/cd.yml/badge.svg)](https://github.com/kalKun24/cert-study-base/actions/workflows/cd.yml)
...
## CD パイプライン（GitHub Actions）

`main` ブランチへのマージをトリガーに、以下の順序でパイプラインが自動実行されます。

```
CI（テスト・リント・セキュリティスキャン）
  ↓ merge to main
CD（Docker ビルド → Artifact Registry プッシュ → Cloud Run デプロイ）
  ├── deploy-backend  : バックエンドをビルド・デプロイ（Cloud Run URL を出力）
  └── deploy-frontend : バックエンド完了後にフロントエンドをビルド・デプロイ
                        （BACKEND_URL / BACKEND_HOST を動的に設定）
```

認証には **Workload Identity Federation** を使用します。サービスアカウントキーはリポジトリに保存しません。

---

### GCP 側セットアップ（初回のみ）

以下のコマンドを GCP プロジェクトオーナーが一度だけ実行してください。

```bash
PROJECT_ID="cert-study-base"
GITHUB_REPO="kalKun24/cert-study-base"
POOL_ID="github-actions"
PROVIDER_ID="github"
SA="cert-study-backend@${PROJECT_ID}.iam.gserviceaccount.com"
...
# Pool の完全リソース名を取得（後続コマンドで使用）
POOL_NAME=$(gcloud iam workload-identity-pools describe "${POOL_ID}" \
  --project="${PROJECT_ID}" \
  --location="global" \
  --format="value(name)")

# 2. Workload Identity Provider を作成（GitHub OIDC）
...
サービスアカウント `cert-study-backend` には以下の IAM ロールが必要です。
...
> JWT_SECRET・SEED_ADMIN_PASSWORD などのアプリシークレットは **GCP Secret Manager** で管理します。GitHub Secrets には登録しません。

---

## 本番環境（Cloud Run）へのデプロイ

CI/CD は GitHub Actions で管理しており、`main` ブランチへのマージをトリガーにテストおよびデプロイが実行されます。デプロイは Cloud Run を対象としています。
