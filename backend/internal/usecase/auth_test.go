...
// --- username / email フォーマットバリデーションのテスト ---

func TestUserUseCase_CreateUser_InvalidUsernameFormat(t *testing.T) {
	tests := []struct {
		name     string
		username string
	}{
		{"短すぎる（2文字）", "ab"},
		{"長すぎる（51文字）", "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"},
		{"使用不可文字を含む（スペース）", "user name"},
		{"使用不可文字を含む（@）", "user@name"},
		{"使用不可文字を含む（日本語）", "ユーザー名"},
		{"空文字", ""},
	}
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if !errors.Is(err, domain.ErrInvalidUsernameFormat) {
				t.Errorf("エラーが期待値と異なります: got %v, want %v", err, domain.ErrInvalidUsernameFormat)
			}
		})
	}
}

func TestUserUseCase_CreateUser_ValidUsername(t *testing.T) {
	tests := []struct {
		name     string
		username string
	}{
		{"3文字（最小）", "abc"},
		{"50文字（最大）", "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa0"},
		{"ハイフンを含む", "user-name"},
		{"アンダースコアを含む", "user_name"},
		{"大文字を含む", "UserName"},
		{"数字を含む", "user123"},
	}
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			// フォーマットエラーが返らないことを確認（重複エラーや他のエラーは許容）
			if errors.Is(err, domain.ErrInvalidUsernameFormat) {
				t.Errorf("有効なユーザー名 %q でフォーマットエラーが返されました", tt.username)
			}
		})
	}
}

func TestUserUseCase_CreateUser_InvalidEmailFormat(t *testing.T) {
	tests := []struct {
		name  string
		email string
	}{
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if !errors.Is(err, domain.ErrInvalidEmailFormat) {
				t.Errorf("エラーが期待値と異なります: got %v, want %v", err, domain.ErrInvalidEmailFormat)
			}
		})
	}
}

func TestUserUseCase_UpdateUser_InvalidEmailFormat(t *testing.T) {
	repo := newMockUserRepository()
	user := testUser("id-1", "alice", "alice@example.com", domain.RoleUser, true)
	repo.addUser(user)
